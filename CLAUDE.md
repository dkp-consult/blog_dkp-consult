...
- `npm run lint` — `eslint --fix` scoped to `pages`, `components`, `lib`, `layouts`, `scripts` (matches `next.config.js` `eslint.dirs`, which `next build` uses for its own pass). `npm run lint:ci` is the same without `--fix`, for CI where the point is to report rather than rewrite. Config lives in `eslint.config.js` (flat config); `eslint-config-next` and `eslint-plugin-prettier` still ship eslintrc presets, so they come in through `FlatCompat`. There are no git hooks — the CI workflow is the gate.
...
- `npm run check` — full gate: `lint` → `test` → `build`. Note it runs `eslint --fix`, so it may rewrite files; use `lint:ci` when that matters. There is no typecheck step (pure JS) and no `format` script. `.github/workflows/check.yml` runs the same three steps on every pull request and on `main`.
...
The plugins are passed through the **`mdxOptions`** key of `bundleMDX`. This matters: mdx-bundler renamed it from `xdmOptions` in v9, and an unknown key is ignored silently — the build stays green while the entire chain never runs. If code stops being highlighted, headings lose their anchors, or images stop going through `next/image`, check that key first. Plugin versions must also track the micromark/unified major that `@mdx-js/mdx` ships; a plugin one major behind fails with `this.getData is not a function`.
